...
// pending_at) arrive as full ISO, so trim to the date part.
const toDateInput = (v?: string | null) => (v ? v.slice(0, 10) : '')

// Fields persisted as first-class case columns. Everything else in the form is
// "extended" and round-trips through the `details` JSON blob so the whole
// new-case form is captured, not just the core columns.
...
const EXTENDED_KEYS = (Object.keys(INITIAL_FORM) as (keyof NewCaseForm)[]).filter(
  (k) => !CORE_KEYS.has(k),
)

/** Serialise the extended form fields into the `details` JSON string. */
...
/**
 * Map an existing case into the rich form shape so the edit page pre-populates
 * exactly the same fields the create page collects — core columns plus every
 * extended field stored in `details`. The create flow derives the title from
 * the first line of the description and saves the whole description as notes,
 * so we reconstruct a description whose first line IS the title.
 */
export function caseToForm(kase: Case): NewCaseForm {
  ...
  return {
    ...
    status: kase.status,
    open_date: toDateInput(kase.date_opened) || TODAY,
  }
}
...
// Shared by the create page (`/cases/new`) and the edit page
// (`/cases/[id]/edit`). In edit mode the caller fetches the case, maps it with
// caseToForm(), and passes it as `initialForm` so every field pre-populates.
export function NewCasePageInner({
  mode = 'create',
  caseId,
  initialForm,
}: {
  ...
  // Map the rich form draft down to the shape the case mutations accept.
  // Unmapped fields (billing, permissions, …) don't persist yet — tracked in
  // `unsavedFields` so we can surface a heads-up toast.
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => router.push(cancelTarget)}>
            Cancel
          </Button>
          {!isEdit && (
            <Button
